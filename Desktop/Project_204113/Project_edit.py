...
def show_go_screen():
    ...
    draw_text(screen, "Adventure!", 50, WIDTH / 2, HEIGHT / 2.5)
    draw_text(screen, 'START: ' , 30, WIDTH / 2, 480)
    draw_text(screen, 'EXIT', 30, WIDTH / 2, 530)
    draw_text(screen, "Move: arrow key to right/lift ", 18, WIDTH / 2, HEIGHT * 2.8 / 4)
    draw_text(screen, "Shoot: space bar ", 18, WIDTH / 2, HEIGHT * 2.5/ 4)
...
background = pygame.image.load(path.join(img_dir, "nebula.png")).convert()
background_rect = background.get_rect()
player_img = pygame.image.load(path.join(img_dir, "playerShip3_orange.png")).convert()
player_mini_img = pygame.transform.scale(player_img, (25, 19))
player_mini_img.set_colorkey(BLACK)
bullet_img = pygame.image.load(path.join(img_dir, "laserGreen13.png")).convert()
...
shoot_sound = pygame.mixer.Sound(path.join(snd_dir, 'Laser_Shoot28.wav'))
shield_sound = pygame.mixer.Sound(path.join(snd_dir, 'Shield6.wav'))
power_sound = pygame.mixer.Sound(path.join(snd_dir, 'Powerup10.wav'))
expl_sounds = []
for snd in ['expl3.wav', 'expl6.wav', 'Explosion7.wav']:
    expl_sounds.append(pygame.mixer.Sound(path.join(snd_dir, snd)))
player_die_sound = pygame.mixer.Sound(path.join(snd_dir, "Explosion11.wav"))
pygame.mixer.music.load(path.join(snd_dir, 'Star_War_-_Soundtrack_(DemoLat.com).mp3'))
